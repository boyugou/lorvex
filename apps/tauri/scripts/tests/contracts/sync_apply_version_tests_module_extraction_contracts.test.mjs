...
const facadePath = path.join(repoRoot, 'lorvex-sync/src/apply/tests/version.rs');
const versionDir = path.join(repoRoot, 'lorvex-sync/src/apply/tests/version');

function read(relativePath) {
  return fs.readFileSync(path.join(versionDir, relativePath), 'utf8');
}
